...
- shallow-clones the `guix-rustup` channel and passes it with `guix shell -L`
  so the `rustup` module resolved by `manifest.scm` is available;
...
module resolved by `manifest.scm` for the pinned nightly Rust toolchain without
running `guix pull` on every benchmark run.
