...
|--------|--------|---------|
| `scripts/guix-wrapper.sh` | compatibility shim | Execs `guix-p2p-wrapper`; persistent setup should use the Rust binary through the `guix-daemon` `GUIX` environment. |
