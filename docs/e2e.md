...
For proof and benchmark modes, the harness maps the fetch policy to
`GUIX_P2P_SUBSTITUTE_ROUTING`: `p2p-only` uses p2p-only routing, while
`p2p-first` and the legacy guix-p2p `http-first` daemon policy route requests to
guix-p2p first so the daemon policy under test is the policy that decides.
