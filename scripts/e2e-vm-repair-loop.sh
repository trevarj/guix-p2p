...
MAX_REPAIRS="${GUIX_P2P_E2E_REPAIR_MAX:-50}"
...
Environment:
  GUIX_P2P_E2E_REPAIR_MAX      Maximum repairs before stopping, default 50
...
while :; do
    ...
    if [ "$repair_count" -ge "$MAX_REPAIRS" ]; then
        log "repair limit reached before repairing $invalid_path; repairs=$repair_count max_repairs=$MAX_REPAIRS"
        exit 1
    fi

    repair_count="$((repair_count + 1))"
    repair_log="$LOG_DIR/store-repair-$repair_count.log"
    log "repair $repair_count starting; path=$invalid_path output=$repair_log"
    # shellcheck disable=SC2086
    if $REPAIR_COMMAND "$invalid_path" 2>&1 | tee "$repair_log"; then
        log "repair $repair_count completed; path=$invalid_path"
    else
        repair_status="$?"
        log "repair $repair_count failed; status=$repair_status path=$invalid_path output=$repair_log"
        exit "$repair_status"
    fi
done
